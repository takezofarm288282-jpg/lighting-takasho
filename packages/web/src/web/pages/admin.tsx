    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          {loginError && (
            <div style={{ color: "#f66", fontSize: 13, marginBottom: 12, textAlign: "center" }}>{loginError}</div>
          )}
          <button
            onClick={login}
            style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button
            onClick={() => token && fetchVisitors(token)}
            style={{ padding: "7px 16px", background: "#2a2a2a", border: "1px solid #444", borderRadius: 6, color: "#aaa", fontSize: 13, cursor: "pointer" }}
          >
            更新
          </button>
          <button
            onClick={logout}
            style={{ padding: "7px 16px", background: "transparent", border: "1px solid #444", borderRadius: 6, color: "#888", fontSize: 13, cursor: "pointer" }}
          >
            ログアウト
          </button>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#222" }}>
                  ...
                  <th style={thStyle}>最終見積</th>
                  <th style={{ ...thStyle, textAlign: "center" }}>見積回数</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((v, i) => (
                    <tr key={v.id} style={{ borderTop: "1px solid #222", background: i % 2 === 0 ? "transparent" : "#1c1c1c" }}>
                      <td style={tdStyle}><span style={{ color: "#555", fontSize: 12 }}>{i + 1}</span></td>
                      <td style={tdStyle}><strong style={{ color: "#eee" }}>{v.name}</strong></td>
                      <td style={tdStyle}><span style={{ color: "#c8a96e", fontFamily: "monospace" }}>〒{v.postalCode}</span></td>
                      <td style={tdStyle}><span style={{ color: "#888", fontSize: 13 }}>{formatDate(v.registeredAt)}</span></td>
                      <td style={tdStyle}><span style={{ color: "#888", fontSize: 13 }}>{formatDate(v.lastEstimateAt)}</span></td>
