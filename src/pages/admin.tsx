...
export default function AdminPage() {
  ...
  const login = async () => {
    setLoginError("");
    if (!backendEnabled()) {
      setLoginError("記録用のGoogleスプレッドシート連携が未設定です（src/config.ts の GAS_URL）");
      return;
    }
    try {
  ...
  const fetchVisitors = async (t: string) => {
    ...
    setError("");
    try {
      const data = await adminVisitors(t);
      if (!data || data.error === "unauthorized") { logout(); return; }
  ...
  const deleteEstimate = async (estimateId: number, visitorId: number) => {
    ...
    try {
      await adminDeleteEstimate(token, estimateId);
      setVisitors((prev) =>
        prev.map((v) =>
          v.id === visitorId
  ...
  const deleteVisitor = async (visitorId: number) => {
    ...
    try {
      await adminDeleteVisitor(token, visitorId);
